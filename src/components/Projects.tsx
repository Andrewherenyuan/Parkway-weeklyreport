...
export default function Projects() {
  ...
  // 加载项目数据：始终以 projects.json 文件为真实数据源，保证每次刷新都用最新的真实图片
  useEffect(() => {
    const loadData = async () => {
      try {
        // 1. 从 projects.json 文件读取（作为真实数据源，每次刷新都使用文件中的最新数据）
        ...
        if (res.ok) {
          ...
          // 2. 只有当 localStorage 中项目数量 > 文件中的数量时（即用户刚新增了项目），才用 localStorage 补充
          const saved = localStorage.getItem('weekly_projects')
          if (saved) {
            try {
              const savedData = JSON.parse(saved)
              if (Array.isArray(savedData) && savedData.length > converted.length) {
                const savedConverted = savedData.map((project: any) => {
                  if (project.image && !project.images) {
                    return { ...project, images: [project.image] }
                  }
                  return project
                })
                setProjects(savedConverted)
                setDataLoaded(true)
                return
              }
            } catch (e) { /* 忽略解析错误 */ }
          ...
          // 3. 否则（数量相等或更少）：始终使用 projects.json 的最新真实数据
  ...
  // ========== 图片压缩 ==========
  // 如果图片超过 2MB，自动压缩到 1MB 以内；返回 { blob, dataUrl }
      ...
      {showNoMoreModal && (
        <div className="fixed inset-0 bg-dark-900/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-dark-700 rounded-2xl p-8 w-full max-w-sm mx-4 text-center">
            ...
            <h3 className="text-xl font-display font-bold text-white mb-2">暂无更多项目</h3>
            <p className="text-white/40 text-sm mb-6">更多精彩内容即将上线</p>
            <button
              onClick={() => setShowNoMoreModal(false)}
              className="w-full py-3 bg-gradient-to-r from-accent-cyan to-accent-purple rounded-xl text-white hover:opacity-90 transition-opacity"
            >
              我知道了
            </button>
          </div>
      ...
      <div className="relative max-w-[1700px] mx-auto px-8 lg:px-16">
        ...
        <div className={`mt-16 text-center transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="flex items-center justify-center gap-4">
            <button 
              onClick={() => setShowNoMoreModal(true)}
              className="px-8 py-3 border border-white/10 text-white/60 text-sm tracking-wider rounded-full hover:border-accent-cyan/50 hover:text-accent-cyan transition-all duration-300"
            >
              查看更多项目
            </button>
